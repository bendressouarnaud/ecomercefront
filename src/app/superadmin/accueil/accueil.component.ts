...
export class AccueilComponent implements OnInit {
  ...
  getAllPartenaies(): void {
    this.meswebservices.getAllPartenaies().toPromise()
      .then(
        resultat => {
          if (resultat.length > 0) {
            this.listePartenaire = resultat;
            this.ident = resultat[0].ident;
          }
        }
      )
  }
  ...
  enregistrerUser(): void {
    if (!this.setModification) {
      this.meswebservices.enregistrerAdminUser(this.nom, this.prenom, this.contact,
        this.email, this.profil, this.ident).toPromise().then(
          resultat => {
      ...
      this.meswebservices.modifierAdminUser(this.nom, this.prenom, this.contact,
        this.email, this.profil, this.ident , this.setUserId).toPromise().then(
          resultat => {
            if (resultat.element == "ok") {
              location.reload();
            }
  ...
  modifieruser(userids: string) {
    ...
    this.listeCommerciauxFul.forEach(
      d => {
        if(d.iduser == userids){
          // Set the FLAGS :
          ...
          this.email = d.email;
          // Get the profil 
          this.listeProfil.forEach(
            p => {
              if(p.libelle == d.profil){
                this.profil = p.idpro;
                return;
              }              
            }
          ...
          this.modes = '';
          this.idDone = '';
          this.idmai = parseInt(d.idmai.toString());
          // Display :
          $('#myModal').modal();
          return;
        }

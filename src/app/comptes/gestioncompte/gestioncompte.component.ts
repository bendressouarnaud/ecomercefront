...
export class GestioncompteComponent implements OnInit {
  ...
  ouvrirzonecom(): void {
    // Open modal :
    this.profil = this.listeProfil[0].idpro;
    this.nom = "";
    this.prenom = "";
    this.email = "";
    this.contact = "";
  ...
  enregistrerUser(): void {
    if (!this.setModification) {
      this.meswebservices.enregistrerUser(this.nom, this.prenom, this.contact,
        this.email, this.profil).toPromise().then(
          resultat => {
      ...
      this.meswebservices.modifierUser(this.nom, this.prenom, this.contact,
        this.email, this.profil, this.setUserId).toPromise().then(
          resultat => {
            if (resultat.element == "ok") {
              location.reload();
            }
